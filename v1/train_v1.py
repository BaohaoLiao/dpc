...
# - Wrapped execution in main() and added multi-GPU device setup support.
...
def main():
    ...
    if KNN_ENABLE:
        ...
        dev_str = (f"cuda:{local_rank}" if (torch.cuda.is_available() and local_rank >= 0) else ("cuda" if torch.cuda.is_available() else "cpu"))
        tmp_model = model.to(dev_str) if str(dev_str).startswith("cuda") else model
        tmp_model.eval()
