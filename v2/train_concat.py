...
class Config:
    ...
    CONCAT_PROB = 0.01  # per-train-row probability to concatenate with a random second pair
...
def _build_arg_parser():
    ...
    p.add_argument("--concat-prob", type=float)
...
def _join_pair_text(a: str, b: str) -> str:
...
def augment_train_pair_concat(
    ...
    *,
    ...
    prefix: str,
    seed: int,
    max_tries_per_row: int = 16,
) -> pd.DataFrame:
    """
    With probability `concat_prob`, create an extra training row by
    concatenating a row (src, tgt) with another randomly sampled row,
    while enforcing source/target tokenizer lengths do not exceed max lengths.
    """
    p = float(concat_prob)
    if p <= 0.0:
        print("[TRAIN_CONCAT_AUG] disabled (CONCAT_PROB <= 0).", flush=True)
        return train_df

    n = int(len(train_df))
    if n < 2:
        print("[TRAIN_CONCAT_AUG] skipped: train set has fewer than 2 rows.", flush=True)
        return train_df

    p = min(max(p, 0.0), 1.0)
    src_max = int(src_max_length)
    tgt_max = int(tgt_max_length)
    pref = "" if prefix is None else str(prefix)
    tries = max(1, int(max_tries_per_row))

    out = train_df.copy()
    base_src = out["transliteration"].astype(str).tolist()
    base_tgt = out["translation"].astype(str).tolist()
    base_src_clean = pre.preprocess_batch(base_src)
    base_records = out.to_dict(orient="records")

    rng = np.random.default_rng(int(seed))
    picked = rng.random(n) < p
    picked_idx = np.flatnonzero(picked)
    ...
    for i in picked_idx:
        ...
        for _ in range(tries):
            ...
            cand_src = _join_pair_text(src_i, base_src[j])
            cand_tgt = _join_pair_text(tgt_i, base_tgt[j])
            cand_src_clean = _join_pair_text(src_clean_i, base_src_clean[j])

            src_ids = tokenizer(pref + cand_src_clean, truncation=False, padding=False)["input_ids"]
            tgt_ids = tokenizer(cand_tgt, truncation=False, padding=False)["input_ids"]

            if len(src_ids) <= src_max and len(tgt_ids) <= tgt_max:
                rec = dict(base_records[i])
                rec["transliteration"] = cand_src
                rec["translation"] = cand_tgt
                new_rows.append(rec)
                added += 1
                applied = True
                break
    ...
    if new_rows:
        out = pd.concat([out, pd.DataFrame.from_records(new_rows)], ignore_index=True)

    print(
        f"[TRAIN_CONCAT_AUG] prob={p:.4f} picked={len(picked_idx)} added={added} "
        f"failed_len={failed} n_before={n} n_after={len(out)}",
        flush=True,
    )
    return out
...
def main():
    ...
    train_aug_df = augment_train_pair_concat(
        train_split_df,
        tokenizer=tokenizer,
        pre=pre,
        concat_prob=float(getattr(Config, "CONCAT_PROB", 0.0)),
        src_max_length=int(getattr(Config, "SRC_MAX_LENGTH", 512)),
        tgt_max_length=int(getattr(Config, "TGT_MAX_LENGTH", getattr(Config, "GEN_MAX_NEW_TOKENS", 512))),
        prefix=str(getattr(Config, "PREFIX", "")),
        seed=int(getattr(Config, "SEED", 42)) + 1703,
    )

    # HF conversion  
    train_for_tokenize = Dataset.from_pandas(train_aug_df, preserve_index=False)
